...
echo --------------------------
echo
echo tctl config clusters set "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-cluster \
  --bridge-address "${DEPLOYMENT_NAME}"-tsb."${DNS_DOMAIN}":8443 --tls-insecure
echo tctl config users set "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-admin \
...
echo tctl config profiles set "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-profile \
  --cluster "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-cluster \
  --username "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-admin
echo tctl config profiles set-current "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-profile
echo
echo ==========================
echo tctl Parameters
echo --------------------------
echo
echo oranization: "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-org
echo tenant: "$DEPLOYMENT_TYPE"-"$DEPLOYMENT_NAME"-tenant
echo
echo user: admin
...
echo
echo =======================================================================
echo possible OpenAPI calls \(if helpers/deploy-openapi.sh script is used\)
echo ======================================================================
echo 
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/1/reviews
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/1/ratings
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/2/reviews
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/2/ratings
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/3/reviews
echo curl -i http://"$DEPLOYMENT_NAME"-bookinfo."$DNS_DOMAIN"/api/v1/products/3/ratings
echo ===============================
echo MP Cluster connection string:
echo
if [[ ${CLUSTER_PLATFORM[${MP_CLUSTER}]} =~ ^(amazon|aws)$ ]]; then
  echo "${AWS_PREFIX}"
  echo
  echo aws eks --region "${REGION_LIST[${MP_CLUSTER}]}" update-kubeconfig --name "${CLUSTER_LIST[${MP_CLUSTER}]}"
else
  echo gcloud container clusters get-credentials "${CLUSTER_LIST[${MP_CLUSTER}]}" --region "${REGION_LIST[${MP_CLUSTER}]}" --project "$GCP_PROJECT"
fi
echo
echo
echo ===============================
